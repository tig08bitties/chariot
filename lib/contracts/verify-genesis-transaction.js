...
async function verifyGenesisTransaction() {
  ...
  try {
    ...
    const tx = await blockscout.getTransaction(GENESIS_TX_HASH);
    ...
    // For contract creation, the contract address is derived from sender + nonce
    ...
    try {
      ...
      if (receipt && receipt.contractAddress) {
        ...
        if (contractAddress.toLowerCase() === EXPECTED_ORACLE_ADDRESS.toLowerCase()) {
          ...
          console.log('   This may be a different deployment or the address was calculated differently');
        }
      }
    } catch (receiptError) {
