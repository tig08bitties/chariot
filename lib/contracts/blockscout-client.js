...
class BlockscoutClient {
  constructor(network = 'ethereum', apiKey = null) {
    this.network = network;
    this.apiKey = apiKey || this.getApiKeyForNetwork(network);
    ...
    // Configure axios with API key interceptor
  ...
  /**
   * Get API key for specific network from environment
   * @param {string} network - Network name
   * @returns {string|null} - API key or null
   */
  getApiKeyForNetwork(network) {
    // Check for network-specific API keys
    if (network === 'arbitrum') {
      return process.env.BLOCKSCOUT_ARBITRUM_API_KEY || null;
    }
