...
class SafeTenderlyIntegration {
  ...
  async simulateSafeTransaction(safeAddress, txParams) {
    try {
      ...
      // Simulate on Tenderly with proper gas parameters
      // Tenderly will auto-detect appropriate gas prices if not provided
      const simulation = await this.tenderly.simulateTransaction({
        ...
        // EIP-1559 gas parameters (auto-detected if not provided)
        maxFeePerGas: txParams.maxFeePerGas,
        maxPriorityFeePerGas: txParams.maxPriorityFeePerGas,
        // Legacy gas_price (fallback for non-EIP-1559)
      ...
      if (error.message.includes('Tenderly')) {
        throw new Error(`Tenderly simulation failed: ${error.message}`);
      }
  ...
  async monitorSafeTransaction(safeAddress, txHash) {
    try {
      ...
      // Enhanced error handling
      if (error.message.includes('Tenderly')) {
        throw new Error(`Tenderly monitoring failed: ${error.message}`);
      }
      if (error.message.includes('Safe')) {
        throw new Error(`Safe operation failed: ${error.message}`);
      }
  ...
  async proposeAndSimulate(safeAddress, txParams, senderAddress) {
    ...
    const safeInstance = await this.safe.getSafe(safeAddress);
